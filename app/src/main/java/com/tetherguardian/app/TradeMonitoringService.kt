...
class TradeMonitoringService : Service() {
    ...
    private fun startMonitoring() {
        ...
        job = scope.launch { while (isActive) { runCatching { monitorOnce() }; delay(10_000L) } }
    }

    private fun stopMonitoring() {
        getSharedPreferences(PREFS, MODE_PRIVATE).edit().putBoolean(KEY_ACTIVE, false).apply()
        job?.cancel(); job = null; stopForeground(STOP_FOREGROUND_REMOVE); stopSelf()
    }
    ...
    private fun monitorOnce() {
        ...
        client.newCall(request).execute().use { response ->
            ...
            for (i in 0 until array.length()) {
                ...
                val time = item.optLong("time", -1L); val price = item.optString("price").toDoubleOrNull(); val volume = item.optString("volume").toDoubleOrNull(); val type = item.optString("type", "unknown")
                if (time >= 0 && price != null && volume != null) trades["$time|$price|$volume|$type"] = Trade(time, price, volume, type)
            }
    ...
    private fun sendStatus(score: Int, buyPct: Double, sellPct: Double, count: Int, count1000: Int, reason: String) {
        sendBroadcast(Intent(ACTION_STATUS_UPDATE).apply { setPackage(packageName); putExtra(EXTRA_SCORE, score); putExtra(EXTRA_BUY_PRESSURE, buyPct); putExtra(EXTRA_SELL_PRESSURE, sellPct); putExtra(EXTRA_TRADE_COUNT, count); putExtra(EXTRA_COUNT_1000, count1000); putExtra(EXTRA_REASON, reason) })
    }
