...
class TradeMonitoringActivity : AppCompatActivity() {
    ...
    private fun bindViews() {
        statusText = findViewById(R.id.marketStateText); scoreText = findViewById(R.id.scoreText); buyPressureText = findViewById(R.id.buyPressureText); sellPressureText = findViewById(R.id.sellPressureText); volumeText = findViewById(R.id.volumeText); speedText = findViewById(R.id.speedText); largeCountText = findViewById(R.id.largeCountText); priceText = findViewById(R.id.priceText); reasonText = findViewById(R.id.reasonText); tradesTable = findViewById(R.id.tradesTable); refreshButton = findViewById(R.id.refreshTradesButton); severeAlertSwitch = findViewById(R.id.severeAlertSwitch)
    }
    ...
    private fun refreshOnce() {
        CoroutineScope(Dispatchers.IO).launch { runCatching { fetchTrades() }.onSuccess { trades -> withContext(Dispatchers.Main) { render(trades) } }.onFailure { error -> withContext(Dispatchers.Main) { statusText.text = "⚪ دریافت داده ناموفق"; reasonText.text = "دلیل: ${error.message ?: "خطای نامشخص"}" } } }
    }
    ...
    private fun fetchTrades(): List<Trade> {
        ...
        client.newCall(request).execute().use { response ->
            ...
            return buildList { for (i in 0 until array.length()) { val item = array.optJSONObject(i) ?: continue; val time = item.optLong("time", -1L); val price = item.optString("price").toDoubleOrNull(); val volume = item.optString("volume").toDoubleOrNull(); val type = item.optString("type", "unknown"); if (time >= 0 && price != null && volume != null) add(Trade(time, price, volume, type)) } }
        }
    }
    ...
    private fun renderTradeRows(rows: List<Trade>) {
        ...
        rows.forEachIndexed { index, trade ->
            val row = TableRow(this); row.setPadding(4, 5, 4, 5)
            ...
            values.forEachIndexed { column, value -> val cell = TextView(this); cell.text = value; cell.textSize = 12f; cell.gravity = android.view.Gravity.CENTER; cell.setPadding(3, 7, 3, 7); cell.setBackgroundColor(if (index % 2 == 0) 0xFFEEF4E9.toInt() else 0xFFF7FAF4.toInt()); val weight = when (column) { 0 -> 1.1f; 1 -> 1.3f; 2 -> 0.9f; else -> 1.2f }; row.addView(cell, TableRow.LayoutParams(0, TableRow.LayoutParams.WRAP_CONTENT, weight)) }
