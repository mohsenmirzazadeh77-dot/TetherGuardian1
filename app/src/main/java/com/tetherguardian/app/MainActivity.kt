...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupDropPercentSpinner() { val labels=dropOptions.map{formatPercent(it)}; val adapter=ArrayAdapter(this,android.R.layout.simple_spinner_item,labels); adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item); dropPercentSpinner.adapter=adapter; val prefs=getSharedPreferences(MonitoringService.PREFS_NAME,MODE_PRIVATE); val saved=prefs.getString(MonitoringService.KEY_DROP_PERCENT,"3.0")?.toDoubleOrNull()?:3.0; val index=dropOptions.indexOfFirst{abs(it-saved)<.001}; if(index>=0) dropPercentSpinner.setSelection(index); dropPercentSpinner.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{override fun onItemSelected(p:AdapterView<*>?,v:View?,pos:Int,id:Long){val x=dropOptions.getOrNull(pos)?:3.0;dropPercentText.text=formatPercent(x);prefs.edit().putString(MonitoringService.KEY_DROP_PERCENT,x.toString()).apply();updateDropLimit(x)};override fun onNothingSelected(p:AdapterView<*>?){} }; dropPercentText.text=formatPercent(dropOptions.getOrNull(if(index>=0)index else 5)?:3.0) }
