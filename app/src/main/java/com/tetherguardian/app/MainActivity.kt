...
class MainActivity : AppCompatActivity() {
    ...
    private val priceReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            val price = intent.getDoubleExtra(MonitoringService.EXTRA_PRICE, 0.0); val base = intent.getDoubleExtra(MonitoringService.EXTRA_BASE_PRICE, 0.0); val baseTime = intent.getLongExtra(MonitoringService.EXTRA_BASE_TIME, 0L); val receivedTime = intent.getLongExtra(MonitoringService.EXTRA_TIME, 0L); val dropLimit = intent.getDoubleExtra(MonitoringService.EXTRA_DROP_LIMIT, 0.0)
            ...
            if (base > 0) baseText.text = formatPrice(base); if (baseTime > 0) baseTimeText.text = "زمان ثبت: ${formatTime(baseTime)}"; if (dropLimit > 0) dropLimitText.text = formatRoundedPrice(dropLimit); if (receivedTime > 0) updateText.text = "آخرین دریافت: ${formatTime(receivedTime)}"; setMonitoringActiveAppearance()
        }
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState); setContentView(R.layout.activity_main); initializeViews(); setupDropPercentSpinner(); setupSoundSpinner(); registerPriceReceiver(); restoreMonitoringState(); monitoringButton.setOnClickListener { toggleMonitoring() }; tradeMonitoringButton.setOnClickListener { startActivity(Intent(this, TradeMonitoringActivity::class.java)) }; soundTestButton.setOnClickListener { testSelectedSound() }; alertPreviewButton.setOnClickListener { previewAlertScreen() }; loadCurrentPrice()
    }

    private fun initializeViews() { connectionText=findViewById(R.id.connectionText); priceText=findViewById(R.id.priceText); updateText=findViewById(R.id.updateText); baseText=findViewById(R.id.baseText); baseTimeText=findViewById(R.id.baseTimeText); dropPercentText=findViewById(R.id.dropPercentText); dropLimitText=findViewById(R.id.dropLimitText); monitoringStatusText=findViewById(R.id.monitoringStatusText); monitoringButton=findViewById(R.id.monitoringButton); tradeMonitoringButton=findViewById(R.id.tradeMonitoringButton); dropPercentSpinner=findViewById(R.id.dropPercentSpinner); soundSpinner=findViewById(R.id.soundSpinner); soundTestButton=findViewById(R.id.soundTestButton); alertPreviewButton=findViewById(R.id.alertPreviewButton) }
    private fun setupDropPercentSpinner() { val labels=dropOptions.map{formatPercent(it)}; val adapter=ArrayAdapter(this,android.R.layout.simple_spinner_item,labels); adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item); dropPercentSpinner.adapter=adapter; val prefs=getSharedPreferences(MonitoringService.PREFS_NAME,MODE_PRIVATE); val saved=prefs.getString(MonitoringService.KEY_DROP_PERCENT,"0.5")?.toDoubleOrNull()?:0.5; val index=dropOptions.indexOfFirst{abs(it-saved)<.001}; if(index>=0) dropPercentSpinner.setSelection(index); dropPercentSpinner.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{override fun onItemSelected(p:AdapterView<*>?,v:View?,pos:Int,id:Long){val x=dropOptions.getOrNull(pos)?:0.5;dropPercentText.text=formatPercent(x);prefs.edit().putString(MonitoringService.KEY_DROP_PERCENT,x.toString()).apply();updateDropLimit(x)};override fun onNothingSelected(p:AdapterView<*>?){} }; dropPercentText.text=formatPercent(dropOptions.getOrNull(if(index>=0)index else 0)?:0.5) }
    private fun setupSoundSpinner() { settingUpSoundSelector=true; val prefs=getSharedPreferences(MonitoringService.PREFS_NAME,MODE_PRIVATE); val title=prefs.getString(MonitoringService.KEY_SOUND_TITLE,null); val label=if(title.isNullOrBlank())"انتخاب صدای هشدار از گوشی" else "صدای انتخاب‌شده: $title"; val adapter=ArrayAdapter(this,android.R.layout.simple_spinner_item,listOf(label)); adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item); soundSpinner.adapter=adapter; soundSpinner.setSelection(0,false); soundSpinner.setOnTouchListener{_,e->if(e.action==MotionEvent.ACTION_UP&&!settingUpSoundSelector)openSoundPicker();true};settingUpSoundSelector=false }
